...
def parse_row(rank: int, cells: list[dict]) -> Optional[KickChannel]:
    ...
    # Example: '<a href="https://streamscharts.com/channels/westcol?platform=kick" ...>'
    # with <span class="t_c_t_c--channel-name">westcol</span> nested inside
    # The href may be relative (/channels/...) or absolute (https://streamscharts.com/channels/...)
    ...
    # Channel name is in a nested span — but for the common case where the slug
    # equals the display name (the usual on Kick), we can fall back to slug.
    # First try to extract the display name from the inner span:
